...
{		
	...
	{
	    $records = Address::with(['city_data', 'state_data', 'county_data'])->get();
	    $this->container['logger']->debug("All addresses query: ", $this->container['db']::getQueryLog());
	    //$records = Address::all();
	    return $response->withJson(["success" => true, "message" => "All Addresses returned", "data" => $records], 200, JSON_UNESCAPED_SLASHES | JSON_PRETTY_PRINT);
	}
	...
	public function readAllAddressesWithFilter($request, $response, $args)
	{
		...
		try {
		    ...
		    $records = Address::with(['city_data', 'state_data', 'county_data'])->where($filter, $value)->get();
		    $this->container['logger']->debug("Address filter query: ", $this->container['db']::getQueryLog());
		    if($records->isEmpty()){
			    return $response->withJson(["success" => true, "message" => "No Address found", "data" => $records], 404);
			}
			return $response->withJson(["success" => true, "message" => "Filtered Addresses by $filter", "data" => $records], 200, JSON_UNESCAPED_SLASHES | JSON_PRETTY_PRINT);
	...
	public function createAddress($request, $response, $args)
	{
		...
		try{
			...
			return $response->withJson(["success" => true, "message" => "Address $recordId has been created."], 200, JSON_UNESCAPED_SLASHES | JSON_PRETTY_PRINT);
		} catch(\Exception $e){
			return $response->withJson(["success" => false, "message" => "Error occured: ". $e->getMessage()], 400);
		}
	}
	...
	public function updateAddress($request, $response, $args)
	{
		...
		try{
			...
			return $response->withJson(["success" => true, "message" => "Updated Address $recordId"], 200, JSON_UNESCAPED_SLASHES | JSON_PRETTY_PRINT);
	...
	public function deleteAddress($request, $response, $args)
	{
		...
		try { 
			...
			return $response->withJson(["success" => true, "message" => "Deleted Address $id"], 200, JSON_UNESCAPED_SLASHES | JSON_PRETTY_PRINT);
		} catch(\Exception $e){
			return $response->withJson(["success" => false, "message" => "Address not found"], 404);
		}
	}
}
