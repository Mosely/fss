...
{		
	...
	public function readAllBranchesOfService($request, $response, $args)
	{
	    ...
	    return $response->withJson(["success" => true, "message" => "All branches_of_service returned", "data" => $records], 200, JSON_UNESCAPED_SLASHES | JSON_PRETTY_PRINT);
	}
	...
	public function readAllBranchesOfServiceWithFilter($request, $response, $args)
	{
		...
		try {
		    ...
		    if($records->isEmpty()){
			    return $response->withJson(["success" => true, "message" => "No branches_of_service found", "data" => $records], 404);
			}
			return $response->withJson(["success" => true, "message" => "Filtered branches_of_service by $filter", "data" => $records], 200, JSON_UNESCAPED_SLASHES | JSON_PRETTY_PRINT);
	...
	public function createBranchOfService($request, $response, $args)
	{
		...
		try{
			...
			return $response->withJson(["success" => true, "message" => "Branch_of_service $recordId has been created."], 200, JSON_UNESCAPED_SLASHES | JSON_PRETTY_PRINT);
		} catch(\Exception $e){
			return $response->withJson(["success" => false, "message" => "Error occured: ". $e->getMessage()], 400);
		}
	}
	...
	public function updateBranchOfService($request, $response, $args)
	{
		...
		try{
			...
			return $response->withJson(["success" => true, "message" => "Updated Branch_of_service $recordId"], 200, JSON_UNESCAPED_SLASHES | JSON_PRETTY_PRINT);
	...
	public function deleteBranchOfService($request, $response, $args)
	{
		...
		try { 
		    ...
		    $record->delete();
			return $response->withJson(["success" => true, "message" => "Deleted Branch_of_service $id"], 200, JSON_UNESCAPED_SLASHES | JSON_PRETTY_PRINT);
		} catch(\Exception $e){
			return $response->withJson(["success" => false, "message" => "Branch_of_service not found"], 404);
		}
	}
}
